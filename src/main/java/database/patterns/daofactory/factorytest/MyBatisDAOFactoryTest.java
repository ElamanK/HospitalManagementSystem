...
public class MyBatisDAOFactoryTest {

    private static final Logger LOGGER = LogManager.getLogger(MyBatisDAOFactoryTest.class);

    public static void main(String[] args) {

        IBaseDAO<Customer> customer = DBFactoryGenerator.getFactory(DBConnectionType.MYBATIS).getDAO("customer");
        ...
        IBaseDAO<Employee> employee = DBFactoryGenerator.getFactory(DBConnectionType.MYBATIS).getDAO("employee");
        List<Employee> listOfEmployees = employee.getAllEntities();
        listOfEmployees.forEach(e -> LOGGER.info(e));
        Employee emp = new EmployeeBuilder().setId(22).setFirstname("Ben").setLastname("Flip").build();
        ...
        IBaseDAO<Transaction> transaction = DBFactoryGenerator.getFactory(DBConnectionType.MYBATIS).getDAO("transaction");
        ...
        IBaseDAO<AccountType> accountType = DBFactoryGenerator.getFactory(DBConnectionType.MYBATIS).getDAO("account type");
